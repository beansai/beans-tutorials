...
| **route_type** | routeType | string | "" | One of DEFAULT, HOLDING, SORTING_RECEIVED, TRAILER, WAREHOUSE_RECEIVED, STORAGE, OMBUDSMAN, RETURN_TO_SENDER, RETURN_TO_HUB, BAD_ADDRESS or an empty string to indicate the type of the route |
...
| **origination** | origination | string | "NOT_SPECIFIED" | The origin of the item, set when the item is created. WF_RETURN_TO_SENDER, MANIFEST_AMEND, MANIFEST_SYNC, MANIFEST_UPLOAD, SCANNED, CALLBACK, DISPATCH, APP, SYSTEM, APP_CREATE, NOT_SPECIFIED |
...
| **cod_amount** | codAmount | string | "" | The amouont to be collected on delivery. Account's own currency is the default if not specified. "12.5", "USD 12.5" are both accepted, where the currency code can be used |
